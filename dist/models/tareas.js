...
class Tareas {
    ...
    guardarTareasDB() {
        return __awaiter(this, void 0, void 0, function* () {
            for (const tarea of this.listadoArr) {
                yield Tarea.upsert(tarea); // Actualiza o inserta la tarea
            }
        });
    }
    // Borrar una tarea
    borrarTarea() {
        return __awaiter(this, arguments, void 0, function* (id = "") {
            if (this._listado[id]) {
                try {
                    yield Tarea.destroy({ where: { id } }); // Borrar de la DB
            ...
            try {
                for (const id of ids) {
                    ...
                    if (tarea && !tarea.completadoEn) {
                        ...
                        yield Tarea.update({ completadoEn: tarea.completadoEn }, { where: { id } }); // Actualiza en la DB
                    }
                }
                ...
                for (const tarea of this.listadoArr) {
                    if (!ids.includes(tarea.id)) {
                        ...
                        yield Tarea.update({ completadoEn: null }, { where: { id: tarea.id } }); // Actualiza en la DB
                    }
                }
            }
            catch (error) {
